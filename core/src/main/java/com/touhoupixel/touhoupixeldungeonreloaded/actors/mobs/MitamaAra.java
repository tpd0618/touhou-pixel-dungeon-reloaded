...
import com.touhoupixel.touhoupixeldungeonreloaded.sprites.MitamaAraSprite;
...
public class MitamaAra extends Mob {

    private static final float SPAWN_DELAY	= 4f;

    {
        spriteClass = MitamaAraSprite.class;
    ...
    public void die( Object cause ) {
        Dungeon.level.drop(new ScrollOfFate(), pos ).sprite.drop();
        Dungeon.level.drop(new ScrollOfFate(), pos ).sprite.drop();
        Dungeon.level.drop(new ScrollOfEarth(), pos ).sprite.drop();
        Dungeon.level.drop(new ScrollOfEarth(), pos ).sprite.drop();
        super.die( cause );
    }
