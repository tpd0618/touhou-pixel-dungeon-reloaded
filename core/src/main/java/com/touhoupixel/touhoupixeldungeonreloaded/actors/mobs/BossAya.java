...
import com.touhoupixel.touhoupixeldungeonreloaded.sprites.BossAyaSprite;
...
public class BossAya extends Mob {
	...
	{
		spriteClass = BossAyaSprite.class;
	...
	public void damage(int dmg, Object src) {
		...
		YokaiMountainBossLevel.State state = ((YokaiMountainBossLevel)Dungeon.level).state();
		...
		if (lock != null) {
			int multiple = state == YokaiMountainBossLevel.State.FIGHT_START ? 1 : 4;
			lock.addTime(dmg*multiple);
		}
		
		//phase 2 of the fight is over
		if (HP == 0 && state == YokaiMountainBossLevel.State.FIGHT_ARENA) {
			...
			Actor.add(new Actor() {
				...
				protected boolean act() {
					Actor.remove(this);
					((YokaiMountainBossLevel)Dungeon.level).progress();
					return true;
				}
			});
			return;
		...
		if (state == YokaiMountainBossLevel.State.FIGHT_START && HP <= HT/2){
			HP = (HT/2);
	...
	private void jump() {
		...
		int newPos;
		if (Dungeon.level instanceof YokaiMountainBossLevel){
			YokaiMountainBossLevel level = (YokaiMountainBossLevel) Dungeon.level;
			
			//if we're in phase 1, want to warp around within the room
			if (level.state() == YokaiMountainBossLevel.State.FIGHT_START) {
				
				level.cleanTenguCell();

				int tries = 100;
				do {
					newPos = ((YokaiMountainBossLevel)Dungeon.level).randomTenguCellPos();
					tries--;
			...
			if (abilityToUse != lastAbility || Random.Int(10) == 0){
				switch (abilityToUse){
					...
					case SHOCKER_ABILITY:
						abilityUsed = throwShocker(BossAya.this, enemy);
			...
			if (timer == 3) {
				...
				for (int cell = 0; cell < PathFinder.distance.length; cell++) {
					...
					if (PathFinder.distance[cell] < Integer.MAX_VALUE) {
						...
						if (ch != null && !(ch instanceof BossAya)) {
				...
				for (int i = area.left; i < area.right; i++){
					for (int j = area.top; j < area.bottom; j++){
						...
						if (cur[cell] > 0 && off[cell] == 0){
							...
							Char ch = Actor.findChar( cell );
							if (ch != null && !ch.isImmune(Fire.class) && !(ch instanceof BossAya)) {
