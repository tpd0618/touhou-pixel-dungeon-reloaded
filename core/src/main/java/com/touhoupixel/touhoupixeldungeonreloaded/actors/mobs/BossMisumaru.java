...
import com.touhoupixel.touhoupixeldungeonreloaded.mechanics.ConeAOE;
import com.touhoupixel.touhoupixeldungeonreloaded.messages.Messages;
import com.touhoupixel.touhoupixeldungeonreloaded.scenes.GameScene;
import com.touhoupixel.touhoupixeldungeonreloaded.sprites.BossMisumaruSprite;
...
public class BossMisumaru extends Mob {

	{
		spriteClass = BossMisumaruSprite.class;
				...
				if (enemy != null && !canReach){
					...
					if (turnsSinceLastAbility >= MIN_COOLDOWN){
						...
						//unless enemy is already stunned, we don't want to stunlock them
							...
							if (sprite != null && (sprite.visible || enemy.sprite.visible)) {
								((BossMisumaruSprite)sprite).slam(enemy.pos);
								return false;
							} else {
								dropRocks(enemy);
	...
	public void move(int step, boolean travelling) {
		...
		if (Dungeon.level.map[step] == Terrain.INACTIVE_TRAP && state == HUNTING) {
			...
			if (RainbowDragonCaveBossLevel.PylonEnergy.volumeAt(pos, RainbowDragonCaveBossLevel.PylonEnergy.class) > 0){
				return;
			}
	...
	public void dropRocks( Char target ) {
		...
		do {
			safeCell = rockCenter + PathFinder.NEIGHBOURS8[Random.Int(8)];
				...
				|| (Blob.volumeAt(safeCell, RainbowDragonCaveBossLevel.PylonEnergy.class) > 0 && Random.Int(2) == 0));
	...
	public void supercharge(){
		supercharged = true;
		((RainbowDragonCaveBossLevel)Dungeon.level).activatePylon();
		pylonsActivated++;

		spend(3f);
		yell(Messages.get(this, "charging"));
		sprite.showStatus(CharSprite.POSITIVE, Messages.get(this, "invulnerable"));
		((BossMisumaruSprite)sprite).updateChargeState(true);
		((BossMisumaruSprite)sprite).charge();
	...
	public void loseSupercharge(){
		supercharged = false;
		((BossMisumaruSprite)sprite).updateChargeState(false);
	...
	protected boolean getCloser(int target) {
		if (super.getCloser(target)){
			...
			if (bestpos != pos){
				...
				Rect gate = RainbowDragonCaveBossLevel.gate;
