...
import com.touhoupixel.touhoupixeldungeonreloaded.actors.mobs.BossMisumaru;
import com.touhoupixel.touhoupixeldungeonreloaded.actors.mobs.Mob;
import com.touhoupixel.touhoupixeldungeonreloaded.actors.mobs.Pylon;
...
public class RainbowDragonCaveBossLevel extends Level {
	...
	public void playLevelMusic() {
		if (locked){
			Music.INSTANCE.play(Assets.Music.FLOOR_13, true);
	...
	public void seal() {
		...
		BossMisumaru boss = new BossMisumaru();
	...
	public void eliminatePylon(){
		...
		for (Mob m : mobs){
			if (m instanceof BossMisumaru){
				((BossMisumaru) m).loseSupercharge();
	...
	public String tileName( int tile ) {
		switch (tile) {
			...
			case Terrain.STATUE:
				...
				return Messages.get(HakugyokurouLevel.class, "statue_name");
			default:
	...
	public String tileDesc( int tile ) {
		switch (tile) {
			case Terrain.WATER:
				return super.tileDesc( tile ) + "\n\n" + Messages.get(RainbowDragonCaveBossLevel.class, "water_desc");
	...
	public Group addVisuals() {
		super.addVisuals();
		RainbowDragonCaveLevel.addCavesVisuals(this, visuals);
		return visuals;
	}
	...
	public static class ArenaVisuals extends CustomTilemap {
		...
		public String name(int tileX, int tileY) {
			...
			if (Dungeon.level.map[i] == Terrain.INACTIVE_TRAP){
				return Messages.get(RainbowDragonCaveBossLevel.class, "wires_name");
			} else if (gate.inside(Dungeon.level.cellToPoint(i))){
				return Messages.get(RainbowDragonCaveBossLevel.class, "gate_name");
			}
		...
		public String desc(int tileX, int tileY) {
			int i = tileX + tileW*(tileY + this.tileY);
			if (Dungeon.level.map[i] == Terrain.INACTIVE_TRAP){
				return Messages.get(RainbowDragonCaveBossLevel.class, "wires_desc");
			} else if (gate.inside(Dungeon.level.cellToPoint(i))){
				if (Dungeon.level.solid[i]){
					return Messages.get(RainbowDragonCaveBossLevel.class, "gate_desc");
				} else {
					return Messages.get(RainbowDragonCaveBossLevel.class, "gate_desc_broken");
				}
			}
	...
	public static class PylonEnergy extends Blob {
				...
				if (energySourceSprite == null){
					for (Char c : Actor.chars()){
						if (c instanceof Pylon && c.alignment != Char.Alignment.NEUTRAL){
							...
							break;
						} else if (c instanceof BossMisumaru){
							energySourceSprite = c.sprite;
						}
					}
					if (energySourceSprite == null){
		...
		public String tileDesc() {
			return Messages.get(RainbowDragonCaveBossLevel.class, "energy_desc");
		}
